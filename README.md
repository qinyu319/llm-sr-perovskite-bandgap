...
3. **Figures, tables, controls, and sensitivity:** rebuild publication assets,
   deterministic controls, and the 507-row group-aware analysis.
...
The 92-row test split is held out but comes from the same curated source. It is
not an independent external literature-validation panel. The model should be
treated as an interpolation-oriented analytical surrogate within the archived
composition domain.
